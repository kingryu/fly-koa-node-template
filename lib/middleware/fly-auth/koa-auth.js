...
const node_env = require('../fly-node-env')
...
const getData = (url, options, log, catchError = false) => {
  return new Promise((resolve, reject) => {
    request(url, options, (err, res, body) => {
      if (err) {
        console.log('koa-auth error', err)
...
module.exports = function (options) {
  const logger = (context) => {
    if (context.log && context.log.debug) {
      return (message, level = 'debug') => {
        context.log[level](message, 'fly-auth')
        debug(message)
      }
    } else {
      return (message) => {
  ...
  return function* (next) {
    ...
    const flyAuth = this.cookies.get('flyauth')
    ...
    if (flyAuth || token) {
      ...
      result = yield getData(config.account + '/account/token/non-validate', {
        ...
        headers: {
          ...
          'user-agent': 'fen-login/jwtToken/1.1'
        },
